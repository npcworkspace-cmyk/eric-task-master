...
Task modules are trusted code, not an untrusted-code sandbox. Install only reviewed local Packs. The observation facade enforces page-mutation/FIFO discipline; it is not a filesystem or credential sandbox for trusted Node.js modules. Before distributing or installing a Pack, apply [Task Pack trust and permissions](task-pack-security.md): declare domains, Profile requirements, local-file access, external effects, extension dependencies, paid services, retention, provenance, and rollback; then review every import and top-level statement against that declaration.
