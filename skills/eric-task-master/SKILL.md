...
Persistent Profiles default to stable local Chrome and `human`. Ephemeral Profiles default to pinned Chromium and `auto`. Both Profile kinds expose `fast`, `auto`, and `human`; the engine is immutable, while behavior remains editable and task start has no behavior override. All three modes keep the same rendered traversal, curved pointer movement and corrections, in-target clicks, per-character keyboard cadence, keyboard selection, segmented scrolling, and reading dwell. Only central timing and guard depth change; `fast` never removes a humanized action, and task code cannot override click or typing pacing. `taskmaster_profiles_update` applies a behavior change to a running task through a Worker-confirmed live control message, without restarting the task. Task status and the Dashboard task card distinguish the selected Profile value from the actual Worker-confirmed configured/effective mode. A registered Task Pack with `full-human-v1` additionally enforces declared transition verification and gates completion on the audited journey. A manual persistent-Profile window is always visible even when its task `headless` setting is enabled.
...
3. Call `taskmaster_tasks_start` once with a stable unique `idempotencyKey` and a `taskLabel` written as action + object + scope, without an Agent name or timestamp; keep the returned task ID and immediately show the returned clickable Dashboard link to the user before following the task.
4. Call `taskmaster_tasks_wait` repeatedly, or `taskmaster_tasks_get` for a snapshot. A cancelled wait does not cancel the durable task. Wait returns early when an Owner ask/modify command arrives; claim the inbox, respond with `taskmaster_task_command_respond`, and apply only a command whose expected revision still matches.
