...
11. The Owner Console is a deliberately small human workbench with three views: task progress and lifecycle controls, Profile management, and Task Pack/executor asset management. Human-readable final reports may appear on task cards; raw artifacts, diagnostics, and Agent coordination remain protocol capabilities rather than Dashboard surfaces.
...
The Dashboard has exactly three primary views: Tasks, Profiles, and Task Packs. Task cards expose the immutable display name, state, current progress, Profile, active execution time, cumulative cooldown time, total elapsed time, applicable lifecycle controls, and the Agent's bounded final report when one exists. It does not fetch or render the Agent Registry, raw artifacts, timeline, diagnostics, or Agent inbox.

A compact header drawer shows only durable human-verification notices and channel settings. The interface switches between Chinese and English without reload; only that language preference is stored in browser local storage. Notification secrets remain write-only in Manager state. Native system notification is enabled when the host probe confirms it is available; Telegram and Feishu/Lark are optional. An alert fires immediately and every 30 seconds until the Owner claims the exact request. Claim is persisted before a best-effort browser focus and never continues the task. Resolution continues the same request only after task revalidation. TaskService is authoritative: if the notification sidecar cannot persist the matching claim/resolve marker after the task transition, the main operation still returns success with an explicit degraded-sync receipt and Manager retries reconciliation.

The Task Packs view groups registered modules into Task Pack, standalone, transient, or protected system assets and also identifies unregistered history/orphan snapshots. It shows purpose, human notes, Agent discoverability, lifecycle, usage counts, last use, file count/size, and a backend-derived deletion decision. Notes and lifecycle changes support bounded batches. Deletion is never a client-side file operation: Manager serializes it against task creation, re-resolves every selected ID, and rejects protected assets, live or cleanup-unsettled task references, and failed resumable tasks only after the current on-disk checkpoint passes integrity validation. Deleted task records and stale/invalid checkpoints do not create phantom blockers; a valid checkpoint recovered after lost IPC still does. CLI one-off standalone installs are transient by default, retire after their first settled task, and are garbage-collected after a seven-day recovery window. Installing a newer semantic Task Pack version retires older versions and a downgrade is rejected.
...
`semantic.snapshot()` builds one bounded, redacted ref/text view across up to sixteen Playwright Frames. A ref resolves only while its snapshot and page URL are current; navigation invalidates it. `semantic.click/fill/navigate` route through the same action/effect policy. Diagnostics persist the semantic view beside the viewport screenshot, allowing an Agent to inspect structure first and use pixels only where structure is ambiguous. Scale preparation fails closed when the snapshot or any frame is truncated, omitted, or unreadable; bounded observation is never represented as complete coverage.

Task-type list responses contain compact metadata (`domains`, `intents`, `tags`, outputs, risk, resume support, interaction contract, Pack provenance, lifecycle, and optional external-cost declaration) without the input schema. The full schema is returned only by describe; a separate MCP read exposes bounded Task Pack lifecycle state without mutation authority. Deprecated task types disappear from ordinary discovery, reject new execution, and may point to one active replacement; their immutable snapshots remain auditable. A Task Pack validates the Human Journey contract and inspects every candidate before one registry commit; conflicts cannot partially install a Pack. Five built-in scaffolds cover `single-page`, `paginated-list`, `list-detail`, `resumable-batch`, and `form-workflow`, and preflight validates modules in isolation without installing them. `surface-probe` is both ordinarily discoverable and the one high-level `taskmaster_scale_prepare` dispatch for requests with no specialized capability. It samples one representative surface, merges bounded main-document, open-shadow and multi-frame semantic signals, fails scale closed when frames cannot be inspected, and requests manual continuation for CAPTCHA/press-and-hold without solving or bypassing it. One bounded pilot must pass before volume increases. This is sampling evidence, not a coverage or authorization claim.

Paid task types declare a currency and task ceiling. Creation requires an explicit smaller budget. Trusted Worker code must reserve each provider operation through a stable idempotency ID before it incurs cost and settle the same ID afterward. Manager serializes these messages and atomically persists the shared task ledger before acknowledging them, so restart, failure, and resume cannot reset the balance. Completion requires no outstanding reservations and exact agreement between final estimated/actual evidence and ledger totals. Public task projections reveal aggregate usage only; provider IDs and ledger entries remain internal. The mechanism fail-closes code that uses the runtime facade but is not a network-level billing firewall for arbitrary trusted module code.
