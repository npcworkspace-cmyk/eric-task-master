...
- `fast`: compressed pacing while a contracted journey still preserves its complete visible action path.
...
Behavior is owned by the Profile and task creation accepts no override. Both Profile kinds expose all three modes. A Profile update is serialized, persisted, sent to its active Worker, and considered successful only after a matching application receipt. The Worker wakes any current pacing delay and applies the mode at the next JavaScript scheduling, pointer, key, or scroll boundary without replacing the task, attempt, Worker, browser, or Profile lease. If acknowledgement fails, the task fails closed instead of silently continuing under a stale policy. A live mode change never cancels a site-required cooldown.

Every `full-human-v1` task type always uses Human Journey mechanics in all three modes: rendered traversal, pointer path, in-target click, per-character input, segmented scrolling, reading dwell, and transition verification remain mandatory. The selected mode controls their pacing and guard depth, not whether they happen.
