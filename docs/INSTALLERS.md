...
Eric Task Master `v3.1.4` is distributed as a CLI-first Manager. Each platform package contains its own pinned Node.js runtime, the production dependency tree, the Manager, CLI, and local Dashboard. Users do not install Node.js, npm, Playwright, or a Playwright browser.
...
The Linux binaries use the official glibc Node.js builds and require glibc 2.28 or newer. Alpine/musl is not a supported `v3.1.4` target. Windows arm64 is not a native `v3.1.4` target. The two macOS packages are deliberately separate because Node.js publishes architecture-specific runtimes; they are not described as a universal binary.
...
- macOS: run `sudo installer -pkg eric-task-master-v3.1.4-macos-<arch>.pkg -target /`, then run `taskmaster panel`. Because this release is unsigned, macOS may require explicit Owner approval.
- Debian/Ubuntu: run `sudo apt install ./eric-task-master-v3.1.4-linux-<arch>.deb`, then run `taskmaster panel`. The portable tarball can be extracted anywhere and started through `bin/taskmaster`.
...
Every target has `eric-task-master-v3.1.4-<target>-portable.zip`. Choose `windows-x64`, `macos-arm64` (Apple silicon), `macos-x64` (Intel), `linux-arm64`, or `linux-x64`. This is a complete Manager runtime, not the separate `eric-task-master-skill-v3.1.4.zip` instructions archive.
...
## Unsigned `v3.1.4` boundary

The repository currently has no Apple Developer ID or Windows Authenticode signing secrets. Therefore `v3.1.4` packages produced by this workflow are explicitly marked `signed: false` in their manifests:
