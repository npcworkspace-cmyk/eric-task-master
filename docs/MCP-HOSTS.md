...
Bootstrap is identical for every host: from the complete project root run `node scripts/taskmaster.mjs connect --json` once and follow its `blockingAction` or `nextAction`. Read the top-level `state` and `readyForTasks`. `agent_host_reload_required` is a hard task gate: keep the upgraded Manager running, reload the current Agent host once, rerun connect, and verify MCP status. A stale or missing bridge version is also rejected by Manager before task routing with `AGENT_HOST_RELOAD_REQUIRED`, so it cannot masquerade as a task-input failure. If diagnosis is needed, run only `node scripts/taskmaster.mjs doctor --json`; do not invent another service. After that, choose one path and keep it for the whole task:
...
| Hermes Agent | native | `mcp_servers` in `$HERMES_HOME/config.yaml` or `~/.hermes/config.yaml` | file adapter; live discovery of the MCP tool surface plus status and Profile-list calls verified |
