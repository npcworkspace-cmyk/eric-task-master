...
export async function runExtensionAcceptance({
  ...
  try {
    ...
    await writeFile(path.join(extensionDir, 'background.js'), [
      ...
      '    let probeTabRemoved = false;',
      '    try {',
      "      if (!Number.isInteger(probeTab?.id)) throw new Error('probe tab id is unavailable');",
      '      protectedTab = await chrome.tabs.get(probeTab.id);',
      '      await chrome.tabs.remove(probeTab.id);',
      '      probeTabRemoved = true;',
      '    } finally {',
      '      if (!probeTabRemoved && Number.isInteger(probeTab?.id)) {',
      '        await chrome.tabs.remove(probeTab.id).catch(() => {});',
      '      }',
      '    }',
      '    const tabRoundTrip = Number.isInteger(probeTab?.id) &&',
      '      protectedTab?.id === probeTab.id && probeTabRemoved;',
      ...
      "    if (declaredPermissions && tabRoundTrip) capabilities.push('tabs');",
