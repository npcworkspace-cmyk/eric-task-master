# Eric Task Master（任务大师）— 全时任务自动化系统

[English](./README.md) | [简体中文](./README.zh-CN.md)

**面向 AI Agent 与人类管理者的全时任务自动化系统。**

版本：**2.8.2**

AI Agent 已经能够理解需求、制定计划和编写代码，但“做出决定”不等于“把任务持续做完”。生产工作需要跨越 Agent 回合持续运行，隔离账号环境，协调多个 Agent，定时汇报进度，在中断后恢复，只在真正需要时请人接手，并留下可以核验的结果。

Eric Task Master 是承接这些工作的控制与执行系统。它把一条 Agent 指令或外部触发转化成可排队、可观察、可恢复的持久任务；即使 Agent 断开，任务仍可继续运行。Playwright 是当前第一执行引擎，但产品本身是围绕任务建立的完整系统：Profile、Worker、队列、进度、检查点、人工接手、恢复、证据、清理与可复用 Task Pack。

`AI Agent 自动化` · `全时自动化` · `无人值守自动化` · `任务编排` · `工作流自动化` · `浏览器自动化` · `Playwright` · `MCP Server` · `长任务` · `多 Agent 自动化` · `RPA`

## 两个产品，两种明确定位

MoneyHand 与 Eric Task Master 不是同一产品的高低版本，而是互相补充的两种能力。

| 产品 | 定位 | 最适合什么时候使用 | 核心承诺 |
| --- | --- | --- | --- |
| [MoneyHand（抓钱手）](https://github.com/npcworkspace-cmyk/npc-moneyhand) | Agent 的浏览器“手” | Agent 要在当前会话中立即、高频地操控 Chromium 浏览器 | 快速连接、直接操作、保持轻量 |
| **Eric Task Master（任务大师）** | 全时任务自动化系统 | 任务需要持续数分钟或数小时、安全排队、跨 Agent 断线继续、协调 Profile 或 Agent、恢复并交付证据 | 一次启动、持续运行、随时可管、结果可证 |

最简单的选择规则：**Agent 现在就要动手，用 MoneyHand；任务离开 Agent 也要继续，用 Task Master。**

## 哪些工作可以交给 Task Master 持续运行？

Task Master 是通用任务系统，不是某一个网站的专用机器人。搭配 Agent、专项 Skill 或 Task Pack，可以用于：
...
这些是可组合的应用方向，不代表内置了所有网站权限，也不代表任何网站都能无条件自动化。Task Master 负责全时控制面与受管执行运行时；专项 Skill 或 Task Pack 负责业务逻辑、平台规则、解析方式、结果格式和任务专属完成证据。
...
| MoneyHand | 立即、直接、高频地操控浏览器 | 有意专注实时操作，不承担完整的持久任务治理 |
| 一次性 CDP 控制器 | 原始控制快、灵活 | 稳定生命周期、队列、进度、检查点、证据、恢复和可复用任务契约 |
| Eric Task Master | 面向生产形态的全时任务自动化 | 网站与行业知识，由 Skill 和 Task Pack 按需叠加 |

Task Master 不替代 Agent 的思考能力，也不是 Agent 的另一双浏览器“手”。它为任务提供一个可以持续存活、运行、恢复并最终交付的系统。

## 三层模型

1. **任务控制面**：Manager 与 Owner Console 通过一个固定本地地址统一管理队列、生命周期、进度、暂停/恢复/取消、人工接手、故障恢复、审计和任务资产。
2. **受管执行面**：隔离的 Playwright Profile 与 Worker 独立运行浏览器任务；Human Journey、检查点、证据和清理机制让执行过程可观察、可恢复。
3. **能力扩展面**：MCP 连接可信本机 Agent；Skill 说明何时、如何调用能力；版本化 Task Pack 把重复业务沉淀成可复用自动化资产，同时与核心解耦。

这样可以运营一套稳定的全时任务系统，再持续定义不同的专属自动化工作单元。
...
只有当任务明确进入“必须真人点击验证”时，Task Master 才会通知用户。系统通知会立即发送，并每 30 秒重复一次，直到用户点击“我已接手”；也可选择接入 Telegram Bot 与飞书。普通失败、卡顿、冷却、清理、完成、登录状态不明确和一般指令都不会通知。接手只会把该任务的浏览器窗口切到前台，不会擅自继续；真人完成页面验证后，再在同一个任务上重新校验并继续。
...
新建持久 Profile 默认使用本机稳定版 Chrome 和 `human`；新建临时 Profile 默认使用项目锁定的 Chromium 和 `auto`。所有 Profile 都可选择 `fast`、`auto` 或 `human`。行为模式在任务运行中切换时，会由 Worker 确认并在下一个调度或运动分段生效，不需要重启任务。Engine 创建后不可变，也不会自动回退。面板还可为每个持久 Profile 单独允许其中已经人工安装的扩展运行；这个设置同时用于以后人工打开和 Agent 任务，只在下一次浏览器启动时生效，不会重启当前任务。扩展运行需要可见浏览器，不能与无头后台模式同时开启；临时 Profile 永不加载或保留扩展。Task Master 不负责安装、复制、同步、读取或登录扩展。

Task Master 的所有运行时动作都进入同一个 Worker FIFO。每个 Journey 步骤会一直占有自己的串行位置，直到可见动作、页面变化校验和稳定等待全部结束，因此并发任务代码也不能让页面动作交错。受信任且实现 `taskmaster-cooperative-v2` 的扩展，会让点击、输入、DOM 修改和导航共享同一队列；请求 ID 在各参与者内部具备幂等语义，授权同时携带参与者与请求身份，跨文档导航会主动释放租约。Task Pack 依赖扩展结果时，运行时会强制执行唯一交接状态机：先武装精确的参与者/请求/操作三元组，完成一个 Task 触发动作，只准对应扩展获得下一次授权，在持有者派生回执处冻结双方，独立核对真实页面后写入与回执绑定的新检查点，最后才接受或拒绝交还。提前、迟到、重叠、错配、未落检查点、过期或未知的交接都会 fail-closed，下一条页面动作绝不抢跑。这个协议只负责协作排队，不认证扩展身份，也不削减扩展已经获得的 Chrome 权限：任意第三方扩展可以正常加载，但浏览器无法强迫其私有代码进入队列，也无法可靠地把它与普通网页脚本区分。操作未接入协议的扩展前，必须先暂停任务并等到 `paused`，随后恢复任务时会重新校验页面。所有任务拿到的 `page` 和 `context` 都只能观察：单文件任务通过 `action` 修改页面，Task Pack 通过 `journey` 修改页面。若底层动作已成功、但后续 Journey 校验失败，运行时会写入持久化未知结果闸门，禁止盲目重放。

任务生命周期调用必须等待完成，不能脱离主流程运行：进度、真人接手、冷却、检查点、截图与扩展交接都必须 `await`。`run()` 返回时会封死迟到入口，Manager 状态此后只会向终态推进。隔离 Worker 在发送完成声明前快照整棵输出目录，Manager 在 Worker 退出后对这份原始快照再做一次完整比对，因此延迟回调既不能复活任务，也不能静默改写交付文件。
...
- 面板只保留任务、Profile、Task Packs 与设置四个清晰区域，并在顶部保留一个紧凑的人工验证通知抽屉。任务历史使用游标分页；可对已选任务批量暂停、恢复、取消或安全删除；任何阻止 Pack 删除的任务都能一键跳转到对应任务卡片。面板不展示容易混淆的 Agent 名录、原始文件、诊断或消息工作台。
...
- Task Packs 资产页会说明每个执行器是做什么的、Agent 能否发现、最后何时使用，以及当前为什么不能删除。用户可以写备注、批量废弃、恢复或安全删除旧资产；已删除的任务记录和无效检查点不会制造幽灵阻塞，系统资产、正在运行或尚未清理的任务资产、确实拥有有效恢复点的资产仍会被保护。
- 面板只展示 Agent 发布的有界最终报告；原始产物、日志与专项交付仍由发起任务的 Agent 负责解释。
...
Task Pack 提供可复用任务类型。它只定义目标、顺序、选择器、平台限速、提取、检查点、输出和完成证据，不再编写鼠标或滚动细节。所有任务都使用只读 Playwright 观察面；单文件任务通过 `action` 修改页面，Pack 则只通过统一的 Human Journey 引擎修改页面。只读 locator 方法仍可不加节奏地读取文本、属性、当前值和有界结构化记录。任意页内 `evaluate` 与直接导入 Playwright 包都不开放，因为别名或第二条客户端路径无法形成可信的只读边界。Worker 会在导入经过审查的 Task 代码前锁定 Playwright 客户端的修改钩子；这是纵深防护，不是针对恶意 Node.js 代码的沙箱。内置单页、分页列表、列表详情、可恢复批处理和表单工作流 5 种生产骨架。没有专项能力却要启动大任务时，Agent 只需调用一次高级工具 `taskmaster_scale_prepare`，由运行时启动内置只读 `surface-probe`，抽样一个代表页面、完成有界扫底与回看、识别阻塞并推荐骨架；只有一个小样任务通过后才放量。专项 Skill 只负责使用时机、输入映射、平台规则和结果解释。

内置探针也会出现在普通任务类型发现结果中。遇到 CAPTCHA 或 press-and-hold 人工验证时，它只发起同任务真人接手并等待，不自动破解、不绕过，也不会把验证交给第三方服务；真人继续后会重新观察，只有阻塞已经消失才允许放量。

Task Master 有意不提供通用的付费供应商预算或计费门面，因为浏览器底座无法证明第三方最终实际扣费。专项流程如需调用付费服务，应由经过审查的 Pack、Skill 与外部业务编排层自行负责授权、预算、幂等、凭证与停止条件，核心运行时不介入。依赖旧计费契约的历史任务仍可只读查看，但不能重新开始或恢复。

CLI 安装的一次性单文件执行器默认是临时资产：第一个任务完成并安全清理后，会自动从 Agent 发现列表退役；保留七天恢复窗口后才进入受保护的自动清理范围。只有明确需要长期复用的单文件模块才使用 `--persistent`，正式复用能力优先做成有版本的 Task Pack。直接打开初始页面或独立工作项的 GET 导航遇到短暂断网，或 429/502/503/504 时，会执行有界退避并把冷却状态展示出来；点击、表单提交、上传等可能产生外部效果的动作绝不会自动重放。
