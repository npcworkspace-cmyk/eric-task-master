...
2. **Owner Console 人类工作台**：一个固定本地网址，保留共享 Profile 管理、任务进度与生命周期控制、精简最终报告，以及 Task Pack 资产目录；资产页会展示用途、备注、使用情况、Agent 可发现状态，并支持批量生命周期管理和有保护的删除。用户直接在隔离的 Playwright 持久 Profile 中登录。
...
- 面板只保留三个清晰区域：任务、Profile 与 Task Packs，不再展示容易混淆的 Agent 名录、原始文件、诊断或消息工作台。
...
- Task Packs 资产页会说明每个执行器是做什么的、Agent 能否发现、最后何时使用，以及当前为什么不能删除。用户可以写备注、批量废弃、恢复或安全删除旧资产；系统资产、正在运行或尚未清理的任务资产、仍可从检查点恢复的资产都会被保护。
