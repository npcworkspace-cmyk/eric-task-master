...
2. **Owner Console 人类工作台**：一个固定本地网址，保留共享 Profile 管理、任务进度与生命周期控制、精简最终报告、Task Pack 资产目录、中英文切换，以及一个紧凑的人工验证通知抽屉。用户直接在隔离的 Playwright 持久 Profile 中登录。
...
- 面板只保留任务、Profile 与 Task Packs 三个清晰区域，并在顶部保留一个紧凑的人工验证通知抽屉；不再展示容易混淆的 Agent 名录、原始文件、诊断或消息工作台。
...
付费 Task Pack 可以声明币种和单任务最高外部成本。任务启动时必须显式批准一个不高于该上限的预算；可信 Pack 在发起每笔付费操作前，必须先通过运行时预留金额，再用同一个稳定操作 ID 结算实际金额。耐久账本由同一任务的全部重试与恢复 attempt 共享，Manager 先落盘再返回授权；存在未结算预留，或最终预估/实际成本证据与账本不一致时，任务都不能完成。公开任务状态只展示币种、预估总额、实际总额和剩余额度。这是对使用成本门面的 Pack 的硬门禁，不是拦截任意第三方网络请求的防火墙。
