...
Eric Task Master 正是为解决这个缺口而开发。它是以 Playwright 为执行核心的全时任务自动化系统，把网页操作变成持久、隔离、可观察、可恢复的任务。Agent 可以启动一个持续数小时的工作，暂时离开后重新接管，通过检查点继续，读取可验证的结果，并确认所有任务窗口已经关闭，而不占用或打扰用户的日常浏览器，也不需要反复消耗 Token 去重新理解同一套控制机制。

MoneyHand（抓钱手）是 Agent 的轻量浏览器“手”，负责即时操作；Task Master 负责让任务在 Agent 离开后仍持续运行。
...
Task Master 不替代 Agent 的思考能力，而是让长时间任务在 Agent 回合结束后仍能持续运行、管理和恢复。

## 三层模型

1. **Task Master 运行时**：纯 Playwright 执行、持久化与临时 Profile、任务队列、进度、恢复、证据、清理，以及统一负责可见网页交互的 Human Journey 引擎。
2. **Owner Console 人类工作台**：一个固定本地网址，保留可分页的任务进度与批量控制、共享 Profile 管理、可直接处理阻塞并强制删除自建陈旧任务或 Task Pack 的资产目录、独立通知设置、中英文切换，以及一个紧凑的人工验证通知抽屉。用户直接在隔离的 Playwright 持久 Profile 中登录。
...
如果任务已经结束，但因为浏览器清理无法证明而锁住持久 Profile，面板会提供仅限 Owner 使用的**强制解除租约**。只要任务或浏览器仍可能存活，Manager 就会拒绝。解除成功后会完整保留 Profile 登录数据，原任务仍保持失败和“清理未确认”记录，同时用新的租约代际阻止旧 Worker 再次占用。
...
- 暂停、恢复、取消和删除记录都会校验任务最新状态。普通删除等待清理完成；Worker 已消失后，Owner 可以强制移除卡死记录。已生成文件仍会保留，也不会让已执行动作变成可重放任务。
- Task Packs 资产页会说明每个执行器是做什么的、Agent 能否发现、最后何时使用，以及当前为什么不能删除。用户可以写备注、批量废弃、恢复、安全删除或强制删除自己的陈旧资产；正在运行的执行器和系统保护资产不能强删。关联任务结果仍会保留，但已删除的代码不能再用于恢复任务。
