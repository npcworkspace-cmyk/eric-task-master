...
行为模式由 Profile 决定，运行中也可切换；任务提交不接受临时覆盖。所有版本化 Task Pack 在三档模式下都强制使用 `full-human-v1` Journey 契约：始终按页面可见路径执行，档位只控制中央运行时执行这条路径的速度与谨慎程度，并为每次成功任务生成 10/10 交互审计。
