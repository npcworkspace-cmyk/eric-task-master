...
- **human**：按渲染页面逐段浏览，使用曲线鼠标移动、目标内点击、键盘输入、分段滚动、阅读停留和页面变化校验；
- **adaptive**：默认快速，遇到动态页面、遮挡、超时、导航不确定、动作失败或限流时暂时转为谨慎或拟人节奏。

拟人节奏是提高交互可靠性的策略，不是指纹伪装，不代表绕过网站控制，也不承诺保护账号免受平台规则影响。
行为模式由 Profile 决定，任务提交不接受临时覆盖。所有版本化 Task Pack 还会强制使用更严格的 `full-human-v1` Journey 契约：具体动作由运行时统一完成，不再由每个 Pack 自己拼一套鼠标与滚动逻辑，并为每次成功任务生成 10/10 交互审计。
...
- 每个任务使用 `Agent-具体任务-创建时间` 的稳定名称，显示当前动作、可视化进度、运行时间、累计冷却时间和总时间。
...
Task Pack 提供可复用任务类型。它只定义目标、顺序、选择器、平台限速、提取、检查点、输出和完成证据，不再编写鼠标或滚动细节。统一的 Human Journey 引擎负责按渲染页面逐段浏览、移动、输入、阅读停留、点击可见翻页控件并校验页面变化；直接绕过的浏览器动作会被拒绝。内置单页、分页列表、列表详情、可恢复批处理和表单工作流 5 种生产骨架，并在注册前完成预检；专项 Skill 只负责使用时机、输入映射、平台规则和结果解释。新逻辑应使用新任务类型名，验收后再把旧类型标记为 deprecated 并指向替代项。
