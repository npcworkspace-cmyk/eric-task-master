...
- Added the Task-facing `extensionFlow` handoff state machine: one exact participant/request/operation expectation must precede one settled Task trigger; only that extension request can run next; its holder-derived receipt freezes both peers; and verified resolution requires a new receipt-linked durable checkpoint. Early, late, overlapping, unknown, mismatched, timed-out, uncheckpointed, or rejected handoffs fail closed and never replay automatically.
- Starts the extension response timer only after the Task trigger settles and internally observes deferred handoff rejections, so slow triggers and late awaits cannot terminate a strict Node Worker before cleanup.
- Persists every declared extension action as a durable unknown effect before delivering its grant, closes that record only after receipt-linked checkpoint verification, and blocks resumed browser mutations when verification or the Worker failed after the extension acted, preventing blind replay across attempts.
- Makes pause/resume share the extension lease boundary: admitted extension work must settle before durable `paused`, awaiting handoff timers freeze while paused, no new extension grant can arrive until the live page passes resume validation, and cancellation during that validation cannot publish a late `running` state.
- Reserves both the Task completion ticket and shared FIFO position synchronously at API admission. Both same-tick arrival directions, long actions, long pauses, pre-seal completion drain, and 64-action Task bursts now have deterministic regressions; trusted Task work no longer inherits extension-request queue or wait limits.
- Serializes and post-write-verifies checkpoint writes, rejects values that JSON would silently erase or alter, prevents a late older checkpoint from overwriting a newer receipt-linked handoff boundary, and synchronously seals checkpoint/read/resolution ingress before completion so delayed fire-and-forget work cannot write after terminal state.
- Freezes checkpoint data and extension-receipt identity at API admission, so a checkpoint queued before an extension receipt cannot later masquerade as post-receipt proof. Resume now gates Journey, runtime capture, and extension admission before any browser call, while effect resolution is sealed and cancellation-safe so an unknown replay barrier cannot be removed by a late callback.
- Replaced the observation surface with a fail-closed positive facade across event listeners, wait predicates, popup/frame/request/response/console/dialog/download/worker handles, `contentFrame`/`owner`, locator handlers, and arbitrary in-page JavaScript evaluation. It blocks locator/page screenshots and PDF capture because they can scroll, fast-forward animations, or dispatch print events outside the FIFO. A new runtime-owned `capture.viewport` writes path-safe, budgeted artifacts through the shared FIFO while preserving page animation and caret state; diagnostics now do the same without in-page resizing.
- Seals and closes pending bridge admissions and in-flight durable effect writes through one lifecycle signal, joins concurrent close callers, and waits for late boundary releases without publishing a grant or receipt after timeout, cancellation, completion, or shutdown.
- Acquires the external pause boundary only when an extension reaches the actual FIFO head, preventing Task/extension/pause cycles from deadlocking while preserving the exact arrival order across 500 seeded mixed-admission rounds.
- Added release-gated guidance identifiers, scaffold coverage, a Pack protocol-forgery rejection, representative real-MV3 `storage`/`tabs`/`scripting`/host-permission proof, and proof that an unintegrated extension remains unrestricted but cannot be falsely claimed as serialized.
- Preserved a terminal attempt's checkpoint-invalid classification across the narrow seal-before-cleanup read race, with a deterministic durability barrier regression.
- Gives every task a read-only Page/Context observation surface: standalone modules route mutations through `action`, while contracted Task Packs route them through `journey`, so delayed raw Playwright callbacks cannot mutate the browser after return.
- Seals progress, human-handoff, cooldown, checkpoint, effect, capture, and action ingress at completion; rejects unawaited handoff/cooldown work; and makes Manager lifecycle state monotonic against late Worker messages after cancellation or completion claim.
- Double-seals the complete output tree with a bounded Worker snapshot immediately before the completion claim and an independent Manager comparison after Worker exit, detecting added, removed, renamed, linked, or rewritten files and directories before publishing completion.
- Makes Locator unwrapping a Worker-owned per-facade capability rather than a public helper, so a Task that imports the observation module cannot recover a raw Locator or bypass the shared action FIFO.
- Binds the observation facade to startup-captured `WeakMap`/`WeakSet` intrinsics, locks Playwright client and shared dispatch mutation hooks before importing a Task, and rejects direct Playwright package imports during Pack preflight. These are defense-in-depth controls for reviewed local Task code, not a hostile Node.js sandbox.
- Prevents late heartbeat floods from cancelling forced cleanup or renewing a Profile lease after cancel/completion, upgrades every restored queued task to the current double-seal contract at launch, and binds completed-artifact digest caching to attempt, expected SHA-256, file identity, size, mtime, ctime, and birth time. Filesystems without a usable ctime simply re-hash instead of trusting a weak cache key.
