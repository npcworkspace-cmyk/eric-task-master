...
The delivery gate runs static boundaries, unit/integration/security tests, real Chromium feature acceptance, and a concurrent/fault/restart commercial acceptance workload. Cross-platform CI is defined for Windows, macOS, and Linux; a platform is considered verified only when its own CI run passes. Releases are manually authorized after the exact `main` commit passes all six jobs, and every version is published once with checksum-protected source, extension, and Skill archives.
