...
Run `npm run version:bump -- patch` (or `minor`, `major`, or an explicit `x.y.z`) to keep package, runtime, and README versions synchronized. `npm run check` rejects drift.
